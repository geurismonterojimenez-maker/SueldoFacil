...
export default function HorasExtrasPrintReport({ directData }: { directData?: any }) {
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="bg-slate-900 text-white text-[9.5px]">
                    <th className="p-1.5 border border-slate-800 rounded-l">Categoría de Horas</th>
                    <th className="p-1.5 border border-slate-800 text-center">Horas</th>
                    <th className="p-1.5 border border-slate-800 text-right">Recargo (%)</th>
                    <th className="p-1.5 border border-slate-800 text-right">Valor Hora Recargada</th>
                    <th className="p-1.5 border border-slate-800 text-right rounded-r">Total Bruto a Pagar</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100 text-slate-700">
                  <tr>
                    <td className="p-1.5 font-semibold">Horas Extras Ordinarias (Hasta 68h semanales)</td>
                    <td className="p-1.5 text-center font-mono">{horasOrdinarias}</td>
                    <td className="p-1.5 text-right font-mono">35%</td>
                    <td className="p-1.5 text-right font-mono">RD$ {(salarioPorHora * 1.35).toLocaleString('en-US', { minimumFractionDigits: 2 })}</td>
                    <td className="p-1.5 text-right font-mono">RD$ {(output.montoExtrasOrdinarias || 0).toLocaleString('en-US', { minimumFractionDigits: 2 })}</td>
                  </tr>
                  <tr>
                    <td className="p-1.5 font-semibold">Horas Extras Extremas (Excedentes de 68h)</td>
                    <td className="p-1.5 text-center font-mono">{horasExtremas}</td>
                    <td className="p-1.5 text-right font-mono">100%</td>
                    <td className="p-1.5 text-right font-mono">RD$ {(salarioPorHora * 2.0).toLocaleString('en-US', { minimumFractionDigits: 2 })}</td>
                    <td className="p-1.5 text-right font-mono">RD$ {(output.montoExtrasExtremas || 0).toLocaleString('en-US', { minimumFractionDigits: 2 })}</td>
                  </tr>
                  <tr>
                    <td className="p-1.5 font-semibold">Horas en Jornada Nocturna (9:00 PM a 7:00 AM)</td>
                    <td className="p-1.5 text-center font-mono">{horasNocturnas}</td>
                    <td className="p-1.5 text-right font-mono">15%</td>
                    <td className="p-1.5 text-right font-mono">RD$ {(salarioPorHora * 0.15).toLocaleString('en-US', { minimumFractionDigits: 2 })}</td>
                    <td className="p-1.5 text-right font-mono">RD$ {(output.montoNocturnas || 0).toLocaleString('en-US', { minimumFractionDigits: 2 })}</td>
                  </tr>
                  <tr>
                    <td className="p-1.5 font-semibold">Horas Trabajadas en Días Feriados o de Descanso</td>
                    <td className="p-1.5 text-center font-mono">{horasFeriadas}</td>
                    <td className="p-1.5 text-right font-mono">100%</td>
                    <td className="p-1.5 text-right font-mono">RD$ {(salarioPorHora * 2.0).toLocaleString('en-US', { minimumFractionDigits: 2 })}</td>
                    <td className="p-1.5 text-right font-mono">RD$ {(output.montoFeriadas || 0).toLocaleString('en-US', { minimumFractionDigits: 2 })}</td>
                  </tr>
                  <tr className="bg-slate-900/5 font-bold text-slate-900">
                    <td className="p-1.5" colSpan={3}>Suma de Ingresos Adicionales</td>
                    <td className="p-1.5 text-right">-</td>
                    <td className="p-1.5 text-right font-mono text-emerald-600">RD$ {totalAdicional.toLocaleString('en-US', { minimumFractionDigits: 2 })}</td>
                  </tr>
                  <tr className="bg-blue-50/40 font-black text-slate-900 text-xs">
                    <td className="p-1.5" colSpan={3}>NUEVO SUELDO BRUTO CALCULADO (Mensual + Extras)</td>
                    <td className="p-1.5 text-right">-</td>
                    <td className="p-1.5 text-right font-mono text-blue-700">RD$ {salarioTotalConExtras.toLocaleString('en-US', { minimumFractionDigits: 2 })}</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <div>
            ...
            <div className="print-no-break grid grid-cols-2 gap-8 border-t border-slate-200 pt-3 mt-3 select-none">
              <div className="text-center">
                ...
                <span className="text-[9px] font-bold text-slate-650 block mt-1.5">Supervisor / Gerente de Área</span>
                <span className="text-[8px] text-slate-400 block">Autorizado y Validado</span>
              </div>
              <div className="text-center">
                <div className="h-10 border-b border-slate-350 mx-auto max-w-[200px]"></div>
                <span className="text-[9px] font-bold text-slate-650 block mt-1.5">Colaborador / Empleado</span>
