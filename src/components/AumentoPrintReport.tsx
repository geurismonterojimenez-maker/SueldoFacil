...
export default function AumentoPrintReport() {
  ...
  useEffect(() => {
    try {
      ...
      if (!stored) {
        stored = localStorage.getItem('sueldofacil_aumento_print');
      }
      if (stored) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 text-slate-650">
        <div className="text-center space-y-2">
          ...
          <p className="text-xs font-bold font-mono">Preparando reporte de Aumento Salarial...</p>
        </div>
      </div>
    );
  }
  ...
  const actualBruto = output.actualBruto || 0;
  const nuevoBruto = output.nuevoBruto || 0;
  const incrementoBruto = output.incrementoBruto || 0;
  
  const actualNeto = output.actualNeto || 0;
  const nuevoNeto = output.nuevoNeto || 0;
  const incrementoNeto = output.incrementoNeto || 0;
  const incrementoAnualNeto = output.incrementoAnualNeto || 0;

  const cambioAfp = output.cambioAfp || 0;
  const cambioSfs = output.cambioSfs || 0;
  const cambioIsr = output.cambioIsr || 0;
      ...
      <div className="max-w-[8.5in] mx-auto mb-6 p-4 bg-white border border-slate-200 rounded-2xl shadow-md flex items-center justify-between print:hidden">
        <div className="flex items-center gap-2.5">
          <span className="text-xs font-bold text-slate-700 flex items-center gap-1.5">
            ...
            Propuesta de Aumento Salarial Lista para Impresión
          </span>
        </div>
